...
The marker pose is fixed via a static TF (`fer_link0 → aruco_marker`). In simulation the pose is set through launch arguments. On the real robot the same static TF must be provided, either from a calibration result or a fixed known pose.
...
All pose values are in the `fer_link0` frame. The default `marker_pitch:=π` rotates the marker so its Z axis points toward the robot (i.e. the marker faces the robot).
...
```
Static TF publisher
(fer_link0 → aruco_marker)
        │
        ▼
aruco_corner_publisher node          ~/next_corner service (std_srvs/Trigger)
  - Computes oMcorner in marker frame          │
  - Publishes WeightedTrajectoryPoint ─────────┘
    to /mpc_input at 100 Hz
        │
        ▼
